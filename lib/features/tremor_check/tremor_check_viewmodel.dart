...
class TremorCheckViewModel extends ChangeNotifier {
  // Backend URLs
  static const String backendUrl = 'http://192.168.1.100:5000'; // UPDATE WITH YOUR IP
  ...
  static const String userId = 'demo-user'; // Replace with real user ID after auth
  ...
  void startTremorTest() {
    ...
    final tremorFreq = random.nextDouble() * 3 + 4; // 4-7 Hz (typical PD tremor)
    ...
    recordingTimer = Timer.periodic(const Duration(milliseconds: 10), (timer) {
      if (isRecording) {
        ...
        final tremorY = baseAmplitude * 0.7 * sin(2 * pi * tremorFreq * t + 0.5);
  ...
  Future<void> analyzeTremorData() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to analyze tremor data: ${response.statusCode}');
      }
  ...
  double _calculateVariance(List<double> values) {
    ...
    final sumSquaredDiff = values.map((x) => pow(x - mean, 2)).reduce((a, b) => a + b);
